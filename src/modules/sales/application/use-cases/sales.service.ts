    ...
    try {
      ...
      await Promise.all(
        products.map((product) =>
          productRepo.update({ id: product.id }, { stockCurrent: product.stockCurrent })
        )
      )
  ...
  private parseCreateSaleInput(dto: Record<string, unknown>): CreateSaleInput {
    ...
    const items: SaleItemInput[] = rawItems.map((item, index) => {
      ...
      return {
        ...
        unitPrice:
          asRecord.unitPrice == null
            ? undefined
            : this.readNumber(asRecord.unitPrice, `items[${index}].unitPrice`)
      }
    })
    ...
    return {
      ...
      discountTotal:
        dto.discountTotal == null ? undefined : this.readNumber(dto.discountTotal, "discountTotal"),
      items
    }
  }
