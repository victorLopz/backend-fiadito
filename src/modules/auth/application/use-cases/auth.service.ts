...
export class AuthService {
  ...
  async verifyOtp(destination: string, code: string, purpose: string): Promise<{ accessToken: string; refreshToken: string }> {
    void { destination, code, purpose };
    return { accessToken: 'access.placeholder', refreshToken: 'refresh.placeholder' };
  }

  async loginWithPassword(identifier: string, password: string): Promise<{ requiresOtp: boolean }> {
