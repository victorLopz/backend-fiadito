...
import {
  OTP_REPOSITORY,
  OtpRepository
} from "src/modules/auth/domain/repositories/otp.repository";
import {
  SESSION_REPOSITORY,
  SessionRepository
...
export class AuthService {
  ...
  async createUser(input: CreateUserInput): Promise<{
    ...
    if (
      normalizedPhone &&
      (await this.userRepository.existsByPhone(normalizedPhone))
    ) {
  ...
  async requestOtp(destinationMobile: string, purpose: string): Promise<void> {
    ...
    await this.otpRepository.invalidateActiveOtps(
      destinationMobile,
      purposeValue
    );
  ...
  async verifyOtp(
    ...
    const purposeValue = this.parseOtpPurpose(purpose);
    const otp = await this.otpRepository.getLatestActiveOtp(
      destinationMobile,
      purposeValue
    );
  ...
  async refreshToken(
    ...
    const token =
      await this.tokenRepository.findActiveRefreshTokenById(tokenId);
    ...
    const user = await this.userRepository.findActiveByIdAndBusiness(
      token.userId,
      token.businessId
    );
    if (!user) {
