...
export class DebtsService {
  ...
  async addPayment(debtId: string, amount: number, userId?: string): Promise<void> {
    ...
    try {
      ...
      const nextStatus = nextBalance === 0 ? DebtStatus.PAID : nextPaid > 0 ? DebtStatus.PARTIAL : DebtStatus.OPEN;
  ...
  async sendDebtReminder(debtId: string): Promise<void> {
    ...
    const client = await clientRepo.findOne({ where: { id: debt.clientId, businessId: debt.businessId } });
